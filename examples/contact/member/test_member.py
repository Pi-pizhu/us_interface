...
class TestMember():
    ...
    '''
    # 参数化的数据
    para_data = load_yaml("../../api_parameter/member_para_data.yml")
    ...
    def test01_add_member(self, userid, name, mobile, errcode, errmsg, add, token):
        logger.info(f"-------开始测试增加成员-------")
        res = self.member.add_member(token, userid, name, mobile)
        logger.info(f"打印响应结果:{res}")
        logger.info("-------测试结束-------")
    ...
    def test02_get_member(self, userid, errcode, errmsg, get, token):
        logger.info("-------开始测试获取成员-------")
        res = self.member.get_member(token, userid)
        logger.info(f"打印响应结果:{res}")
        logger.info("-------测试结束-------")
        assert res["errcode"] == errcode
        assert errmsg in res["errmsg"]
    ...
    def test04_delete_member(self, userid, errcode, errmsg, delete, token):
        logger.info("-------开始测试获取成员-------")
        res = self.member.delete_member(token, userid)
        logger.info(f"打印响应结果:{res}")
        logger.info("-------测试结束-------")
        assert res["errcode"] == errcode
        assert errmsg in res["errmsg"]
    ...
    def test05_multi_delete_member(self, userid_list, errcode, errmsg, multi_delete, token):
        logger.info("-------开始批量删除获取成员-------")
        res = self.member.multi_delete_member(token, userid_list)
        logger.info(f"打印响应结果:{res}")
        logger.info("-------测试结束-------")
        assert res["errcode"] == errcode
        assert errmsg in res["errmsg"]
    ...
    def test03_edit_member(self, userid, name, mobile, errcode, errmsg, edit, token):
        logger.info("-------开始修改获取成员-------")
        res = self.member.edit_member(token, userid, name, mobile)
        logger.info(f"打印响应结果:{res}")
        logger.info("-------测试结束-------")
        assert res["errcode"] == errcode
        assert errmsg in res["errmsg"]
    ...
    def test_active_stat(self, date, errcode, errmsg, token):
        logger.info("-------开始查看企业微信活跃度-------")
        res = self.member.get_active_stat(token, date)
        logger.info(f"打印响应结果:{res}")
        logger.info("-------测试结束-------")
        assert res["errcode"] == errcode
        assert errmsg in res["errmsg"]
    ...
    def test_get_invite_qr(self, size, errcode, errmsg, token):
        logger.info("-------开始获取企业微信二维码-------")
        res = self.member.get_invite_qr(token, size)
        logger.info(f"打印响应结果:{res}")
        logger.info("-------测试结束-------")
        assert res["errcode"] == errcode
        assert errmsg in res["errmsg"]
    ...
    def test_get_depart_member(self, department_id, fetch_child, errcode, errmsg, token):
        logger.info("-------开始获取部门成员简单的信息-------")
        res = self.member.get_depart_member(token, department_id, fetch_child)
        logger.info(f"打印响应结果:{res}")
        logger.info("-------测试结束-------")
        assert res["errcode"] == errcode
        assert errmsg in res["errmsg"]
    ...
    @pytest.mark.parametrize(("department_id,fetch_child,errcode,errmsg"), depart_explicit_data,
                             ids=depart_explicit_ids)
    def test_get_depart_member_explict(self, department_id, fetch_child, errcode, errmsg, token):
        logger.info("-------开始部门成员详细信息-------")
        res = self.member.get_depart_member_explicit(token, department_id, fetch_child)
        logger.info(f"打印响应结果:{res}")
        logger.info("-------测试结束-------")
        assert res["errcode"] == errcode
        assert errmsg in res["errmsg"]
    ...
    @allure.story("增加联系人")
    @pytest.mark.smoke
    def test_all_smoke_member(self, test_all_pre_data, token):
        add_res = self.member.add_member(token, "tong1234", "tong1234", "13172771165")
        edit_res = self.member.edit_member(token, "tong1234", "tong1234", "13172771165")
        del_res = self.member.delete_member(token, "tong1234")
        multi_del_res = self.member.multi_delete_member(token, ["tongtong1", "tongtong2", "tongtong3"])
        qr_res = self.member.get_invite_qr(token, 1)
        active_res = self.member.get_active_stat(token, "2020-10-10")
        depart_res = self.member.get_depart_member(token, "1", "1")
        depart_res_e = self.member.get_depart_member_explicit(token, "1", "1")
