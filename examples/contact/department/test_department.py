...
class TestDepartment():
    ...
    # 获取参数化的数据
    para_data = load_yaml("../../api_parameter/para_department.yml")
    ...
    def test_add_depart(self, token, parentid, name, errcode, errmsg, add_depart):
        logger.info("-------------开始增加部门测试---------")
        res = self.depart.add(token, parentid, name)
        logger.info("-------------测试结束---------")
    ...
    def test_get_depart(self, token, id, errcode, errmsg):
        logger.info("-------------开始获取部门测试---------")
        res = self.depart.get(token, id)
        logger.info("-------------测试结束---------")
        assert res["errcode"] == errcode
        assert errmsg in res["errmsg"]
    ...
    def test_edit_depart(self, token, id, name, errcode, errmsg, edit_depart):
        logger.info("-------------开始编辑部门测试---------")
        res = self.depart.edit(token, id, name)
        logger.info("-------------测试结束---------")
        assert res["errcode"] == errcode
        assert errmsg in res["errmsg"]
    ...
    def test_delete_depart(self, token, id, errcode, errmsg, delete_depart):
        logger.info("-------------开始删除部门测试---------")
        res = self.depart.delete(token, id)
        logger.info("-------------测试结束---------")
        assert res["errcode"] == errcode
        assert errmsg in res["errmsg"]
    ...
    def testa_add_depart(self, token, depart_smoke):
        logger.info("-------------开始冒烟增加部门测试---------")
        res = self.depart.add(token, 1, "smoke")
        logger.info("-------------测试结束---------")
    ...
    def testb_get_depart(self, token, depart_smoke):
        logger.info("-------------开始冒烟获得部门测试---------")
        res = self.depart.get(token, 3)
        logger.info("-------------测试结束---------")
        assert res["errcode"] == 0
    ...
    def testc_edit_depart(self, token, depart_smoke):
        logger.info("-------------开始冒烟编辑部门测试---------")
        res = self.depart.edit(token, 4, "edit")
        logger.info("-------------测试结束---------")
        assert res["errcode"] == 0
    ...
    def testd_delete_depart(self, token, depart_smoke):
        logger.info("-------------开始删除部门冒烟测试---------")
        res = self.depart.delete(token, 5)
        logger.info("-------------测试结束---------")
        assert res["errcode"] == 0
